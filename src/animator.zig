...
fn make_tag_map(comptime T: type, aseprite: tatl.AsepriteImport, map: *std.AutoHashMap(T, FrameSlice)) !void {
    for (0..aseprite.tags.len) |i| {
        const tag = aseprite.tags[i];
        if (std.meta.stringToEnum(T, tag.name)) |tag_name| {
...
pub fn Animator(comptime T: type) type {
    return struct {
        ...
        tag_map: std.AutoHashMap(T, FrameSlice),
        canvas_size: rl.Vector2,

        pub fn load(aseprite: tatl.AsepriteImport, allocator: std.mem.Allocator) !Animator(T) {
            ...
            try make_tag_map(T, aseprite, &map);
            return .{
        ...
        pub fn get_frames(self: @This(), key: T) []AnimationFrame {
